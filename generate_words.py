...
def main():
    ...
    batch_size = 100        # Process 100 words per API call
    delay_between_calls = 0.5  # Optimized for pay-as-you-go billing

    # 1. Guarantee output file exists immediately (fixes Git 'pathspec' error)
    ...
    # 2. Read existing progress to resume seamlessly
    if os.path.exists(output_file):
        try:
            with open(output_file, "r", encoding="utf-8") as f:
                existing_data = json.load(f)
                processed_words = {item["word"].lower() for item in existing_data if "word" in item}
    ...
    for i in range(0, total_remaining, batch_size):
        ...
        print(f"\n[Batch {current_batch}/{total_batches}] Processing {len(batch)} words...")
        ...
        try:
            ...
            # If quota is hit, save current work and exit with code 0 so GitHub Actions commits progress
            if "RESOURCE_EXHAUSTED" in err_msg or "429" in err_msg:
                print("\nQuota reached. Saved current progress to words.json and exiting cleanly.")
                sys.exit(0)
            else:
                print("Pausing 5 seconds before retrying next batch...")
                time.sleep(5)
